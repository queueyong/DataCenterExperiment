...
function workload_setter(arrival_type::String)
  if arrival_type == "Stationary"
    WS = Workload_Setting[]
    push!(WS, Workload_Setting(20.0, LogNormal(log(0.25),sqrt(log(2+1))), 0.25, 2.0,
                                      LogNormal(log(5.0),sqrt(log(1+1.5))), 5.0, 1.5,
                                      t->t ) )

    push!(WS, Workload_Setting(20.0, LogNormal(log(0.5),sqrt(log(1.5+1))), 0.5, 1.5,
                                      LogNormal(log(10.0),sqrt(log(1+2))), 10.0, 2.0,
                                      t->t ) )

    push!(WS, Workload_Setting(20.0, Exponential(4), 0.25, 1.0,
                                      LogNormal(log(5.0),sqrt(log(1+1))), 5.0, 1.0,
                                      t->t ) )

    push!(WS, Workload_Setting(20.0, LogNormal(log(2.0),sqrt(log(0.8+1))), 0.1, 0.8,
                                      LogNormal(log(2.0),sqrt(log(0.8+1))), 2.0, 0.8,
                                      t->t ) )

    push!(WS, Workload_Setting(15.0, LogNormal(log(3.0),sqrt(log(0.5+1))), 0.2, 2.0,
                                      LogNormal(log(3.0),sqrt(log(0.5+1))), 3.0, 0.5,
                                      t->t ) )
    return WS
  elseif arrival_type == "Time-varying"
    WS = Workload_Setting[]
    push!(WS, Workload_Setting(20.0, LogNormal(log(0.25),sqrt(log(2+1))), 0.25,2.0,
                                      LogNormal(log(5.0),sqrt(log(1+1.5))), 5.0,1.5,
                                      t->4.0-3*sin((π/12)*t))  )
    ...
    push!(WS, Workload_Setting(20.0, LogNormal(log(0.5),sqrt(log(1.5+1))), 0.5, 1.5,
                                      LogNormal(log(10.0),sqrt(log(1+2))), 10.0, 2.0,
                                      t->2.0-1.5*sin((π/12)*t))  )

    push!(WS, Workload_Setting(20.0, Exponential(4), 0.25, 1.0,
                                      LogNormal(log(5.0),sqrt(log(1+1))), 5.0, 1.0,
                                      t->4.0-2.5*sin((π/12)*t))  )
...
function find_min(two_dim_array::Array{Array{Float64}})
  min = typemax(Float64)
  index = 0
  for i in 1:length(two_dim_array)
    if temp > two_dim_array[i][1]
      temp = two_dim_array[i][1]
      index = i
    end
  end
  return (index, min)
...
function generate_RP(d::Any, length::Any)
  N = rand(d,1)
  if typeof(length) == Float64
    while N[end] < length
      push!(N, N[end] + rand(d))
    end
...
function generate_NHNP(λ::Function, d::Any, length::Any)

end

# when the terminal criterion is "Time"
function arrival_generator(arrival_type::String, WS::Array{Workload_Setting}, REPLICATION_TIME::Float64)
  if arrival_type == "Stationary"
    RP = Array{Float64}[]
    for ws in WS
      push!(RP, generate_RP(ws.inter_arrival_distribution, REPLICATION_TIME*1.3))
    end
    AI = Arrival_Information[]
    i, app_type, time = 1, 0 , 0.0
    while time < REPLICATION_TIME*1.3
      app_type, time = find_min(RP)
      push!(AI, Arrival_Information(i,app_type,time,rand(WS[app_type].workload_distribution),typemax(Float64)))
      shift!(RP[app_type])
      i += 1
    end
    return AI
...
arrival_generator("Stationary", WS, 20.0)
WS = workload_setter("Stationary")
scv(WS[5].inter_arrival_distribution)

# when the terminal criterion is "the number of arrivals"
function arrival_generator(arrival_type::String, WS::Array{Workload_Setting}, MAX_ARRIVALS::Int64)
  if arrival_type == "Stationary"
