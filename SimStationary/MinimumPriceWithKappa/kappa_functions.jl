...
function setOptimalPolicy(dc::DataCenter)
    ...
    speed_sol, routing_sol = solveCentralized(dc.SS,dc.WS,dc.S,use_kappa=true)

    # set optimal speed for servers
    for j in 1:length(speed_sol)
        dc.S[j].current_speed = dc.S[j].κ
        dc.S[j].previous_speed = dc.S[j].κ
...
function p_dot(j::Int64, S::Array{Server})
  if S[j].current_price >= 0.0
    return S[j].κ + S[j].current_remaining_jobsize - S[j].current_speed
  else
    return max(S[j].κ + S[j].current_remaining_jobsize - S[j].current_speed, 0.0)
  end
end
